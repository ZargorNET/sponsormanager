...
const _items: Ref<Array<{ value: string, focused: boolean }>> = ref([{value: "", focused: false}]);
...
function updateCount() {
    ...
    if (newPossible())
        _items.value.push({value: "", focused: false});
}
...
function newPossible(): boolean {
    ...
    return !(valuesList.length != 0 && valuesList[valuesList.length - 1].value === "");
}

function updateEmit() {
    updateCount();
    emits('update:items', _items.value.map(item => item.value).filter(value => value !== ""));
}

    ...
    <div class="overflow-auto max-w-full">
        <n-space size="small">
            <n-tag :bordered="false" size="small" v-for="tag in tags" v-if="!edit">
                <NuxtLink :to="`/search/%23${encodeURI(`${tag}`)}`" class="overflow-hidden text-ellipsis">
                    {{ tag }}
                </NuxtLink>
            </n-tag>
            <template v-else>
                <n-button dashed v-for="(tag, index) in tags" size="tiny"
                          @click="propIndexToEdit = index; showModal = true">
                    {{ tag }}
                </n-button>
                <n-button type="primary" dashed size="tiny" @click="addTag()">+</n-button>

                <n-modal v-model:show="showModal"
                         style="width: 600px; position: fixed; top: 400px; right: 50%; left: 50%; transform: translate(-50%, -50%);"
                    ...
                    <template #default>
                        ...
                        <div class="flex justify-center child:mx-1 mt-4">
                            <div>
                                <n-button type="primary" @click="showModal = false">Ok</n-button>
                            </div>
                            <div>
